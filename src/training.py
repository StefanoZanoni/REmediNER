...
class TrainerNer:
    ...
    def _run_epoch_ner(self, train_data, epoch, model, optimizer, scheduler):
        ...
        for ids, masks, labels in train_data:
            ...
            loss_batch += run_batch_ner(ids, masks, labels, model, optimizer, scheduler)
    ...
    def train_ner(self, train_data, model, optimizer, scheduler):
        ...
        for epoch in range(self.epochs):
            epoch_loss_mean = self._run_epoch_ner(train_data, epoch, model, optimizer, scheduler)
    ...
    def kfold_cross_validation(self, k):
        ...
        for fold, (train_idx, val_idx) in enumerate(kfold.split(self.train_data)):
            ...
            train_loader = DataLoader(self.train_data, batch_size=self.batch_size, pin_memory=True,
                                      shuffle=False, sampler=DistributedSampler(train_subsampler))
            val_loader = DataLoader(self.train_data, batch_size=self.batch_size, pin_memory=True,
                                    shuffle=False, sampler=DistributedSampler(val_subsampler))
